...
function parseRoleIdFromCommand(msg, kind) {
  ...
  let reg = /^#?(?:终末地|zmd)(?:抽卡记录|抽卡纪录|记录)\s*([0-9]{5,})\b/i
  if (kind === "refresh") {
    reg =
    ...
    reg = /^#?(?:终末地|zmd)(?:角色抽卡记录|角色抽卡纪录|角色记录|角色纪录)\s*([0-9]{5,})\b/i
  } else if (kind === "show_weapon") {
    reg = /^#?(?:终末地|zmd)(?:武器抽卡记录|武器抽卡纪录|武器记录|武器纪录)\s*([0-9]{5,})\b/i
  }
...
export class gachalog extends plugin {
  constructor(e) {
    ...
    super({
      ...
      rule: [
        ...
        { reg: "^#?(?:终末地|zmd)(?:角色抽卡记录|角色抽卡纪录|角色记录|角色纪录)(?:\\s*.*)?$", fnc: "showChar" },
        { reg: "^#?(?:终末地|zmd)(?:武器抽卡记录|武器抽卡纪录|武器记录|武器纪录)(?:\\s*.*)?$", fnc: "showWeapon" },
        { reg: "^#?(?:终末地|zmd)(?:抽卡记录|抽卡纪录|记录)(?:\\s*.*)?$", fnc: "show" },
      ],
    })
  }
  ...
  async help() {
    ...
    const lines = [
      ...
      ``,
      `查看：${p}记录 / ${p}记录1234567890 / ${p}记录 @用户（兼容：${p}抽卡记录）`,
