...
- 明日方舟（`#fz`）：森空岛签到 / 全部签到 / 抽卡记录工具链（更新、全量、导入、导出、删除、查看）
- 其他：状态统计、更新日志、环境诊断
...
- `#zmd卡池`
- `#zmd<名称>图鉴`
- `#zmd<名称>介绍` / `#zmd<名称>技能` / `#zmd<名称>天赋` / `#zmd<名称>潜能`
- `#zmd<名称>专武` / `#zmd<名称>武器`
...
- `#fz抽卡记录` / `#fz抽卡记录 @用户`（按“限定/常驻/中坚”聚合展示）
...
1) 提示缺少依赖 `qrcode` / `node-fetch` / `yaml` / `puppeteer`
   - 在 TRSS-Yunzai 根目录执行：`pnpm add qrcode node-fetch yaml puppeteer`，然后重启。
